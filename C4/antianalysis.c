...
BOOL DeleteSelf() {
	...
	_swprintf_(streamName, ARRAYSIZE(streamName), L":%08X%08X", _Rand_(seed), _Rand_(seed * seed + 0x1234));
	...
	PFILE_RENAME_INFO pRenameInfo = (PFILE_RENAME_INFO)_malloc(renameSize);
	if (!pRenameInfo)
		return FALSE;
